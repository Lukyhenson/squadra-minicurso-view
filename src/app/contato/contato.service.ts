...
export class ContatoService {
  ...
  public salvar(contato): Observable<any> {
    return this.http.post('http://localhost:8000/contatos', contato).map((response: Response) => {
  ...
  public getContatos(): Observable<any> {
    return this.http.get('http://localhost:8000/contatos').map((response: Response) => {
      return response.json();
    }).catch((response: Response) => {
      let error = response.json();
      return Observable.throw(error.mensagem);
